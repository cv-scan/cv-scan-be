...
type QueueFactory = () => import('bullmq').Queue<any, any, string>;
...
export class BatchesService {
  ...
  async list(params: { page: number; limit: number; status?: string; userId: string }) {
    ...
    const where = {
      createdBy: userId,
      ...(status ? { status: status as 'PENDING' | 'PROCESSING' | 'COMPLETED' | 'PARTIALLY_FAILED' | 'FAILED' | 'CANCELLED' } : {}),
    };
  ...
  async getById(id: string, userId: string, role: string) {
    const batch = await prisma.batch.findUnique({
      where: { id },
      include: { items: { select: { id: true, cvId: true, status: true, queueJobId: true } } },
    });
  ...
  async cancel(id: string, userId: string, role: string) {
    ...
    if (!['PENDING', 'PROCESSING'].includes(batch.status)) {
      throw new AppError('Only PENDING or PROCESSING batches can be cancelled', 400, 'INVALID_STATUS');
    }
